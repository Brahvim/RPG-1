...
struct List* listDouble(struct List *const p_list, size_t const p_count) {
	size_t capacity = p_list->capacity;

	while (unlikely(capacity <= p_list->size + p_count)) {

		capacity *= 2;

	}

	// p_list->data = realloc(p_list->data, p_list->stride * capacity);
	reallocArrayStride(p_list->data, capacity, p_list->stride);
